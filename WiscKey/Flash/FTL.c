...
void WritewithFTL(AccessType_t AccessType,flash_size_t StartCluster,flash_size_t Length){
		//* initialize parameter about FTL simulation
		...
		//*  End of initializing parameter about FTL simulation
		
		//* Start to simulate operation on FTL
		flash_size_t StartLBA;	//* start LBA
		StartLBA = StartCluster;
		flash_size_t EndLBA;	//* end LBA
		flash_size_t len;		//* the number of LBAs in each read/write opeartion
		EndLBA = StartLBA - 1 +Length;
		StartCluster = StartLBA/PageToLBARatio;
		Length = EndLBA/PageToLBARatio-StartCluster+1;
		...
		flash_size_t NowCluster;
		for(NowCluster=StartCluster; NowCluster<(StartCluster+Length); NowCluster++)
		{
			if(AccessType == WriteType) 
			{
				SecureFTLWriteOneCluster(NowCluster);	//* Write a cluster to the flash memory and also update the FTL table. We suggest this function to write a cluster to the flash memory and also update the FTL table
			}
			else if(AccessType == ReadType) 
			{
				...
				if(FTLReadOneCluster(NowCluster) == False) { //* Read a cluster to the flash memory and also update the FTL table. We suggest this function to write a cluster to the flash memory and also update the FTL table
		...
		if(AccessType == WriteType) {
			...
			if(NowCluster == (StartCluster+Length)) {
		...
		//* End of simulating operations on FTL
}
#endif
