...
inleveldb_set(DB * db, std::string &key, std::string &value)
{
  ...
  static bool
inleveldb_get(DB * db, std::string &key, std::string &value)
{
  assert(lldb);
  ReadOptions ropt;
...
void logfile_GC_simulation(std::string &accvalue,std::string &key,std::string &value){
	...
	while(i<=goal)
	{
		...
		//access key
	  	...
	  	bool found = inleveldb_get(recdbptr,GCkey,value);
	  	if(found){
	  		//append to logfile
	  		size = 32;
		  	{	
		  		//fwrite(&key,size,1,logfile);
		  		//fwrite(&accvalue,size,1,logfile);
		  	}

	  		//insert to LSM tree
	  		value = std::to_string(GC);
	  		inleveldb_set(recdbptr,GCkey,value);
	  		//head tail update
	  		tail += 64;
	  		head += 64;
	  		value = std::to_string(head);
	  		inleveldb_set(recdbptr,headkey,value);
	  		value = std::to_string(tail);
	  		inleveldb_set(recdbptr,tailkey,value);
	  	}else{
	  		//do nothing
	  	}
...
Status DB::Open(const Options& options, const std::string& dbname,
  ...
  *dbptr = NULL;
  ...
  std::string accvalue;
  accvalue = std::to_string(head);
  inleveldb_set(*dbptr, headkey, accvalue);
  accvalue = std::to_string(tail);
  inleveldb_set(*dbptr, tailkey, accvalue);
  accvalue = std::to_string(GC);
  inleveldb_set(*dbptr, GCkey, accvalue);
  inleveldb_get(*dbptr, headkey, accvalue);
  printf("db_impl:%s",accvalue);
  */
  wisckey = true;
  return s;
}
